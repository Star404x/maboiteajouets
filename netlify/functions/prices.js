...
exports.handler = async (event, context) => {
  console.log(`[prices] ${event.httpMethod} request`);
  ...
  try {
    if (event.httpMethod === "GET") {
      console.log("[prices] GET - Fetching prices from DB");
    ...
    if (event.httpMethod === "POST") {
      console.log("[prices] POST - Updating price");
      const apiKey = event.headers["x-api-key"];

      if (apiKey !== adminKey) {
        console.warn("[prices] Unauthorized");
        return {
          statusCode: 401,
      ...
      if (!id || typeof price !== "number" || price < 0) {
        return {
          statusCode: 400,
          body: JSON.stringify({ success: false, error: "Invalid id or price" }),
        };
      }

      console.log(`[prices] Updating ${id} to €${price}`);
      const client = await pool.connect();
